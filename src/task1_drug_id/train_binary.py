"""Task 1 binary baseline — drug vs no-drug at triage (tier 1).

Predicts: ``ground_truth_drug != 0`` → positive class "drug-positive"
(Kraken + Triton + Coral combined) vs negative class "None" (typical
medical pathology). This is the tier-1 question implied by the
organizers' Task1_Two_Tier_Input_Data.csv: is this patient on ANY
festival drug? The 4-class tier-2 ("which drug?") lives in
``train_baseline.py``.

Runs BOTH evaluation modes:
  - 5-fold stratified cross-validation (all 261 patients)
  - Temporal holdout (train on days < last, test on the last day)

Source of truth: ``derived/outcomes.csv`` (no leakage from
features_triage.csv).

Outputs:
  derived/task1_binary_baseline_summary.csv      (CV summary)
  derived/task1_binary_oof_predictions.csv       (CV OOF preds)
  derived/task1_binary_temporal_summary.csv      (holdout summary)
...
# Binary classes: 0 = No drug (None), 1 = Drug-positive (K/T/C)
CLASSES = ["No drug", "Drug-positive"]
...
def load_data() -> tuple[pd.DataFrame, np.ndarray, pd.Series]:
    ...
    y = (df["ground_truth_drug"] != 0).astype(int).to_numpy()
...
def binary_metrics(y_true: np.ndarray, p_pos: np.ndarray) -> dict:
    """All the metrics we report for a binary classifier."""
    ...
    # Confusion-matrix-derived sensitivity / specificity / PPV / NPV
    ...
    return {
        ...
        "roc_auc": auc,
        "pr_auc": prauc,
        "log_loss": float(log_loss(y_true, np.clip(p_pos, 1e-12, 1 - 1e-12),
                                    labels=[0, 1])),
        "brier": brier,
        "bss": bss,
        "sensitivity": sens,
...
def run_cv() -> None:
    print("=" * 78)
    print("Task 1 binary — 5-fold stratified CV (drug vs no-drug)")
    print("=" * 78)

    X_df, y, _ = load_data()
    print(f"X: {X_df.shape}   y: {y.shape}")
    print(f"Class distribution:  No drug = {int((y == 0).sum())}  "
          f"Drug-positive = {int((y == 1).sum())}")
    print(f"Prevalence (positive): {y.mean():.3f}")
    ...
    for name, _ in model_zoo().items():
        ...
        m_mean = {k: float(np.mean([m[k] for m in fold_metrics]))
                  ...
                  and not k.startswith("t") and not k.startswith("f")}
        m_std = {k: float(np.std([m[k] for m in fold_metrics]))
                 for k in m_mean}

        print(f"\n--- {name} ---")
        print(f"  5-fold means (+/- std):")
        for k in ("log_loss", "accuracy", "roc_auc", "pr_auc",
                   "brier", "bss", "sensitivity", "specificity",
                   "ppv", "npv"):
            print(f"    {k:14s}  {m_mean[k]:.4f}  (+/- {m_std[k]:.4f})")

        row = {"model": name, "prevalence": float(y.mean()),
                **{f"{k}_mean": m_mean[k] for k in m_mean},
                **{f"{k}_std": m_std[k] for k in m_mean}}
        rows.append(row)
    ...
    # Save
    summary.to_csv(DERIVED / "task1_binary_baseline_summary.csv")
    ids = pd.read_csv(DERIVED / "features_triage.csv")["encounter_id"]
    oof = pd.DataFrame({
        "encounter_id": ids.values,
        "true_label": [CLASSES[int(v)] for v in y],
    })
    for name in model_zoo():
        oof[f"p_drug_{name}"] = oof_proba[name]
    oof[f"pred_label_{best}"] = [CLASSES[int(v)] for v in
                                   (oof_proba[best] >= 0.5).astype(int)]
    oof.to_csv(DERIVED / "task1_binary_oof_predictions.csv", index=False)
    print(f"\nSaved: derived/task1_binary_baseline_summary.csv")
    print(f"       derived/task1_binary_oof_predictions.csv")
...
def run_temporal() -> None:
    print("\n" + "=" * 78)
    print("Task 1 binary — temporal holdout (train on early days, "
          "test on last day)")
    print("=" * 78)

    X_df, y, arrival = load_data()
    ...
    print(f"Train prevalence: {y[is_train].mean():.3f}")
    print(f"Test prevalence:  {y[is_test].mean():.3f}")
    ...
    rows = []
    pred_rows = []
    ids = pd.read_csv(DERIVED / "features_triage.csv")["encounter_id"].values
    ...
    summary.to_csv(DERIVED / "task1_binary_temporal_summary.csv")
    pd.DataFrame(pred_rows).to_csv(
        DERIVED / "task1_binary_temporal_predictions.csv", index=False)
    print(f"\nSaved: derived/task1_binary_temporal_summary.csv")
    print(f"       derived/task1_binary_temporal_predictions.csv")
