"""Task 1 — Triton vs Coral binary classifier.

Companion to ``train_kraken_binary.py``. Given a patient is
drug-positive AND not Kraken, is it **Triton** (CNS depressant with
cardiac awareness) or **Coral** (hallucinogen)?

Cohort: ``ground_truth_drug in {2, 3}`` → Triton-or-Coral subset
(n = 51 + 48 = 99). Target encoding:
  1 = Triton Tabs  (positive, n=51, prevalence ~52%)
  0 = Coral Dust   (negative, n=48)

**Hard problem by construction**. v6 shows Triton and Coral share
many triage-time characteristics: near-normal HR / temp / labs,
both lean against diaphoretic + tremor + agitation. Their defining
tokens diverge only post-triage:
  Triton:  "ringing in ears", "palpitations", "psychomotor slowing"
  Coral:   "time distortion", "perceptual alteration",
           "spatial disorientation"
None of those appear in ``triage_brief_note`` (they live in HPI /
MDM). Expected ceiling: AUC in the 0.5-0.65 range.

Runs both 5-fold stratified CV and temporal holdout, same model
lineup as the other Task-1 scripts (logreg, rforest, hgb).

Outputs:
  derived/task1_triton_coral_baseline_summary.csv
  derived/task1_triton_coral_oof_predictions.csv
  derived/task1_triton_coral_temporal_summary.csv
...
# This script's positive = Triton (2); negative = Coral (3).
CLASSES = ["Coral Dust", "Triton Tabs"]
...
def load_data() -> tuple[pd.DataFrame, np.ndarray, pd.Series, np.ndarray]:
    ...
    # Cohort: Triton (2) or Coral (3) only — exclude None and Kraken.
    n_before = len(df)
    df = df[df["ground_truth_drug"].isin([2, 3])].reset_index(drop=True)
    print(f"Cohort filter (Triton or Coral): {n_before} -> {len(df)} patients")

    drop = ["encounter_id", "encounter_arrival_date", "ground_truth_drug"]
    drop = [c for c in drop if c in df.columns]
    y = (df["ground_truth_drug"] == 2).astype(int).to_numpy()  # 1=Triton
...
def run_cv() -> None:
    print("=" * 78)
    print("Task 1 — Triton vs Coral, 5-fold stratified CV")
    print("=" * 78)
    X_df, y, _, ids = load_data()
    print(f"X: {X_df.shape}   y: {y.shape}")
    print(f"Class distribution:  Coral = {int((y == 0).sum())}  "
          f"Triton = {int((y == 1).sum())}")
    print(f"Prevalence (Triton): {y.mean():.3f}")
    maj = int(y.mean() >= 0.5)
    ...
    summary.to_csv(DERIVED / "task1_triton_coral_baseline_summary.csv")
    oof = pd.DataFrame({
        "encounter_id": ids,
        "true_label": [CLASSES[int(v)] for v in y],
    })
    for name in model_zoo():
        oof[f"p_triton_{name}"] = oof_proba[name]
    oof[f"pred_label_{best}"] = [CLASSES[int(v)] for v in
                                   (oof_proba[best] >= 0.5).astype(int)]
    oof.to_csv(DERIVED / "task1_triton_coral_oof_predictions.csv",
                index=False)
    print(f"\nSaved: derived/task1_triton_coral_baseline_summary.csv")
    print(f"       derived/task1_triton_coral_oof_predictions.csv")
...
def run_temporal() -> None:
    print("\n" + "=" * 78)
    print("Task 1 — Triton vs Coral, temporal holdout")
    print("=" * 78)
    ...
    print(f"Train Triton prevalence: {y[is_train].mean():.3f}")
    print(f"Test  Triton prevalence: {y[is_test].mean():.3f}")
    ...
    summary.to_csv(DERIVED / "task1_triton_coral_temporal_summary.csv")
    pd.DataFrame(pred_rows).to_csv(
        DERIVED / "task1_triton_coral_temporal_predictions.csv",
        index=False)
    print(f"\nSaved: derived/task1_triton_coral_temporal_summary.csv")
    print(f"       derived/task1_triton_coral_temporal_predictions.csv")
